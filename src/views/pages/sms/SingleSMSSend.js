...
const schema = yup.object().shape({
  ...
  phone: yup
    .string()
    .min(5, obj => showErrors('Phone', obj.value.length, obj.min))
    .required(),
  message: yup
    .string()
...
const SingleSMSSend = () => {
  ...
  const onSubmit = async data => {
    ...
    const URL = '/dashboard/sendSingleAdminSMS'
    ...
    try {
      ...
      if (response?.data.status === '200') {
        ...
        setSeverity('success')
        setOutputValue('SMS successfully sent')
        setOpen1(true)
              ...
              <Controller
                ...
                render={({ field: { value, onChange } }) => (
                  <TextField
                    ...
                    onChange={onChange}
                    fullWidth
                    placeholder='Sender ID (e.g Church Name)'
                    error={Boolean(errors.sender)}
                  />
                )}
              />
              {errors.sender && <FormHelperText sx={{ color: 'error.main' }}>{errors.sender.message}</FormHelperText>}
            </FormControl>
          </Grid>
